...
//cast từ domain model sang view model
  @Override
  public List<QLProduct> getProducts(int position, int pageSize) {
    _lstQlProduct = new ArrayList<>();
    List<Product> product = _iProductRepository.findAll(position, pageSize);
  ...
  public QLProduct getProductById(long id) {
    Product x = _iProductRepository.findById(id);
  ...
  public QLProduct createNewProduct(QLProduct product) {
    product.setId(null);
    Product x = _iProductRepository.save(new Product(product.getId(), product.getName(), product.getPrice(), product.getCategory()));
    return new QLProduct(x.getId(), x.getName(), x.getPrice(), x.getCategory());
  }
